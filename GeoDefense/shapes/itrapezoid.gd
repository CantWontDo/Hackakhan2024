...
func _ready():
	...
	top_right = Vector2(snapped_pos.x + top_length * grid.current_frequency, snapped_pos.y)
	
	bottom_left = Vector2(snapped_pos.x, snapped_pos.y + height * grid.current_frequency)
	bottom_right = Vector2(snapped_pos.x + bottom_length * grid.current_frequency, snapped_pos.y + height * grid.current_frequency)
...
func draw_guides():
	...
	draw_guide_text(str(top_length), top_length, Vector2.DOWN  * 1.2 * grid.current_frequency + Vector2.RIGHT)
	...
	draw_dashed_line(Vector2(snapped_pos.x + grid.current_frequency * 2, snapped_pos.y), 
		Vector2(snapped_pos.x + grid.current_frequency * 2, snapped_pos.y + (height) * grid.current_frequency), Color.BLACK, 3)
	draw_guide_text(str(bottom_length), bottom_length,
		(Vector2.DOWN * (height) * grid.current_frequency) + Vector2.RIGHT)
	pass
func get_area() -> float:
	return abs((top_length + bottom_length) * height / 2.0)

func cursor_in() -> bool:
	var is_in : bool = false
	var looker = global_position + real_pos
	var biggest = bottom_length if bottom_length > top_length else top_length
	if (grid.cursor_pos.x > looker.x 
	and grid.cursor_pos.y > looker.y
	and grid.cursor_pos.x < looker.x + biggest * grid.current_frequency
